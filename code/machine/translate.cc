...
// cout << vpn << endl;
    if (vpn > pageTableSize) {
        ...
        return AddressErrorException;
                ...
                // if we have a useful physical  memory
                 if(j<NumPhysPages){ 
                 ...
                 // if we don't have a useful memory
                            ...
                            // temporary buffer for saving Page to Physical memory and
                            // saving Page to Syncdisk
                             char *buf1 = new char[PageSize];
                             char *buf2 = new char[PageSize];

                            // algorithm to get victim page
                             //Fifo
                            if( isFIFO ){
                                victim = fifo % 32;
                                ++fifo;
                     ...
                     kernel->vm_Disk->ReadSector(pageTable[vpn].virtualPage, buf2);
                     bcopy(buf2,&mainMemory[victim*PageSize],PageSize);
                     kernel->vm_Disk->WriteSector(pageTable[vpn].virtualPage,buf1);
                     
                     // set our victim page into the invalid status
                     mainMemTable[victim]->virtualPage=pageTable[vpn].virtualPage;
                     mainMemTable[victim]->valid=FALSE;
        
                     // save the page which we want to use  to the phsical memory
                     pageTable[vpn].physicalPage=victim;
                     pageTable[vpn].valid = TRUE;
                     mainMemTable[victim]=&pageTable[vpn];
            }
    }
    entry = &pageTable[vpn];
    } else {
